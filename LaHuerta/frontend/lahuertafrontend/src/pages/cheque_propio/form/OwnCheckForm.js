...
const OwnCheckForm = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const loadedBanks = await loadOptions(bankUrl, (data) =>
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (id) {
        await axios.put(`${ownCheckUrl}${id}/`, payload);
      } else {
        await axios.post(ownCheckUrl, payload);
      }
      navigate('/own-check');
    } catch (error) {
          ...
          <SectionCard icon={<CalendarTodayIcon sx={{ fontSize: 20 }} />} title="Fechas" cols={3}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <BasicDatePicker
                ...
                hasError={touched.dueDate && Boolean(errors.dueDate)}
              />
